...
var anchors = function () {
    return function (tree, vFile) {
        ...
        visit(tree, function (node, idx, parent) {
            var _a;
            if (node.type === 'heading' && !((_a = node.data) === null || _a === void 0 ? void 0 : _a.anchorAdded)) {
                if (!node.data) {
                    node.data = {
                        anchorAdded: true,
                    };
                ...
                var title = node.children.filter(function (c) { return ['text', 'inlineCode', 'SvpHeading'].includes(c.type); }).map(function (c) { return c.value; }).join('');
                var slugId_1 = title.replace(/[&#$?* :/]/g, '-');
                if (anchors.some(function (an) { return an.slugId === slugId_1; }))
                    slugId_1 += '_';
